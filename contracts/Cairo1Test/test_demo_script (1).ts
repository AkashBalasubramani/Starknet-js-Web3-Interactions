...
import { Provider, Contract, json, Account, ec, stark, uint256 } from "starknet"
import fs from "fs"
import hardhat from "hardhat"
import {
  StarknetContractFactory,
  Account as HardhatAccount,
  StarknetContract,
} from "hardhat/types/runtime"
const { getSelectors, getFunctionNames } = require("./diamond.js")
...
async function run() {
  ...
  compiledLoan = json.parse(
    fs
      .readFileSync(
        "./starknet-artifacts/contracts/modules/loan.cairo/loan.json"
      )
      .toString("ascii")
  )
  compiledIntegration = json.parse(
    fs
      .readFileSync(
        "./starknet-artifacts/contracts/modules/integration.cairo/integration.json"
      )
      .toString("ascii")
  )
  console.log("Run working 1")
  ...
  /// Deployer address
  diamondAddress =
    "0x664cbf4b140b0c6cfcdf744b6c62b5f826c39b8b0bb22f72b0603a0f2dc6486"
  ...
  const withdraw_partial_loan_amt = uint256.bnToUint256(
    new BN("3500000000000000")
  )
